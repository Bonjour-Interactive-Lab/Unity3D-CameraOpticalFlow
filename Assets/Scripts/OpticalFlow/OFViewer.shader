    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                float2 colsrows     = float2(100, 100);
                colsrows.y          /= aspect; 
                ...
                float2 uvsampleOF   = (iuv + offset * 0.0) / colsrows;

                float2 sampleOF     = tex2D(_OF, uvsampleOF).rg;

                fuv             = fuv * 2.0 - 1.0;
                float direction = 0.0;
                if(length(sampleOF) > 0.25){
                    //top line
                    direction       = sdSegment(fuv, float2(0, 0), sampleOF);
                ...
                return float4(src.rgb * 0.25 + smoothstep(0.1, 1.0, abs(of)) * .5 + direction, 1);
            }
            ENDCG
        }
    }
